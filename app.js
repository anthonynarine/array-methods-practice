...
//Map, Filter
const veggies = cuisineName.filter((element) =>{
  return element.includes("Vegetarian")
});
...
//Filter
const containsChickpea= dishes.filter((element1) =>(
  element1.ingredients.includes("chickpea")
));

//10. Create a function that will return the total serving count of all dishes.
//Must use Reduce, not a loop.
const serving = dishes.map((element2) =>{
  const totalServings = element2.servings;
  return totalServings;
});
const totalServingOfAllDishes = serving.reduce((total, currentVal) => {
  return total + currentVal;
})

//11. Create a function that will return an array of any objects that do not share a cuisine type with any other objects.
dishes.filter((item) =>{
  if(item.cuisine !== item.cuisine) return 
})
